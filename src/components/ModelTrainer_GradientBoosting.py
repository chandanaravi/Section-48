...
class ModelTrainer_GradientBoosting:
    def __init__(self):
        logger.info("ModelTrainer_Gradient Boosting initialized.")
        self.model_config=ModelTrainer_Config()
        self.model=None
        
        data_dir_path=os.getcwd() + "/artifacts"
        current_datetime = datetime.datetime.now()
        filename_str ="GradientBoosting_Model_Obj_" + current_datetime.strftime("%Y-%m-%d_%H-%M-%S")
    ...
    def TrainModel_and_Save(self,X_train,y_train):
        model,para=self.model_config.get_model_with_params("Gradient Boosting")
